...
#y_alphabets = break_points_gaussian(y_alphabet_size).tolist()
    ...
    def x_distrubted_values(self,series):
        mean=np.mean(series)
    ...
    """-------------     Segmentization  Data      ------------- """    
    def segment_ts(self,series,windowSize=window_size,skip_offset=skip_offset):
        ts_len=len(self.x1)
        ...
        for i in range(0, ts_len):
            ...
            curr_word=self.alphabetize_ts(sub_section) 
    ...
    """-------------     Alphabetize      ------------- """    
    def alphabetize_ts(self,sub_section):
        mean_val=self.x_distrubted_values(sub_section)
        y_alpha_val=min(self.y_alphabets, key=lambda x:abs(x-mean_val))
        y_alpha_idx=self.y_alphabets.index(y_alpha_val)
        curr_word = self.index_to_letter(y_alpha_idx)
    
        return(curr_word)


    """-------------     Complete Words      ------------- """    
    def complete_word(self,series=x1,word_len=word_lenth,skip_len=0):
        alphabetize= self.segment_ts(series)
        complete_word=list()
    ...
    """-------------     Simlliar Words      ------------- """    
    def simillar_words(self):
        simlliarWords= self.complete_word()
        sax = defaultdict(list)
        for i in range(0,len(simlliarWords)):
            if(len(simlliarWords[i])==self.word_lenth):
                sax[simlliarWords[i]].append(self.skip_offset*(i))
    ...
    """-------------     Visualization      ------------- """  
    def visualize(self,data,alph_size,lent,key):
        row=int(lent/4)
        print(key)
        if(lent > 4):
        ...
        if(width>=3):
            for i in range(len(df)):
                for j in range(len(df)):
                    ...
                    eucl_dist= eucl_dist/math.sqrt((self.word_lenth))
    ...
    def  distance_calculation (self):
        i=0
        simillar_word=self.simillar_words()
        sax_keys =list(simillar_word.keys())
        sax_values =list(simillar_word.values())
        alphabet_size=self.window_size+(self.skip_offset*(self.word_lenth-1))
        for n_val in sax_values:
            ...
            for n1_val in n_val:
                ...
                while (alpha_count < alphabet_size):
                    x2.append(self.x1[n1_val+alpha_count])
                    alpha_count=alpha_count+1
            #aa=self.visualize(x2,alphabet_size,len(n_val ),key)
        
            temp_list=self.split(x2,alphabet_size)
            temp_df = pd.DataFrame(temp_list)
            temp_df.insert(loc=0, column='start', value=n_val)
            self.dist_matrix(temp_df,key) 
...
#instance=sax_visualization()
#instance.distance_calculation()
